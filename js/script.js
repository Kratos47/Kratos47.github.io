...
function loadArcadeGame(element) {
    ...
    // CRITICAL FIX: scrolling="no" physically prevents the browser from generating a scrollbar inside the iframe
    $("#game-iframe").attr({
        "src": url,
        "scrolling": "no" 
    });
    ...
    $stage.stop(true, true).fadeIn(400).css("display", "flex");

    // PLAYABILITY SCROLL: Calculate middle of screen to ensure game fits
    var viewportHeight = $(window).height();
    var stageHeight = $stage.outerHeight();
    var stageTop = $stage.offset().top;
    
    var targetScroll = stageTop - (viewportHeight / 2) + (stageHeight / 2);
...
function closeArcadeGame() {
    $("#game-stage").stop(true, true).fadeOut(300);
